...
/**
 * Workspace creation form. Two entry points share this page:
 *
 *   1. Fresh sign-ups with 0 contexts. WorkspaceSelector's auto-redirect
 *      lands them here (per resolveDestination's `create_workspace` branch).
 *   2. Existing users clicking "Create new workspace" in the AppShell
 *      header dropdown. HeaderWorkspaceSwitcher's createNew handler routes
 *      here.
 *
 * Same form for both paths. Both end at /w/[new-slug] with the new
 * workspace as the active one and last_active_workspace_id persisted.
 *
 * Atomicity comes from a Postgres RPC (security definer) that does both
 * inserts inside one transaction. Slug generation + collision suffixing is
 * handled by the workspaces_auto_slug trigger — clients don't pre-check.
 */
export default function CreateWorkspacePage() {
  ...
  // this page is reachable by direct URL, so guard against the "auth
  // forgotten / signed out in another tab" case.
  useEffect(() => {
