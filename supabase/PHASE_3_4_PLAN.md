...
5. **Launch-blocker: session inactivity timeout — requires Supabase Pro.** CLAUDE.md → Security model → Auth methods documents "Long-lived session (30 days) for clients so they don't constantly re-auth." This is currently UNCONFIGURED — sessions today persist until the user signs out or clears localStorage (effectively indefinite). **Discovered during the polish pass:** the dashboard control for this lives at Authentication → Sessions, and Supabase gates the entire Sessions configuration panel behind the Pro plan. On free tier, "Time-box" and "Inactivity timeout" fields are read-only with the message "Configuring user sessions is only available on the Pro Plan and above." Earlier brief framing of "5-minute pre-launch dashboard click" was incorrect — this is now a launch-blocker, not a polish item. **Path forward: upgrade Supabase project to Pro before MVP launch, then configure inactivity timeout = 30 days.** Pre-customer risk of unbounded sessions is acceptable. Alternative paths considered + rejected: (a) client-side inactivity tracking on a `profiles.last_active_at` column — more code, brittle, doesn't enforce on the auth layer; (c) upgrade-now-as-precaution — defers spend without immediate need. Going with defer-to-launch-prep.
